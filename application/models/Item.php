...
class Item extends CI_Model
{
	...
	*/
	...
	public function get_search_suggestions($search, $filters = array('is_deleted' => FALSE, 'search_custom' => FALSE, "category"=>""), $unique = FALSE, $limit = 25)
	{
		$suggestions = [];
		$non_kit = array(ITEM, ITEM_AMOUNT_ENTRY);

		$this->db->select($this->get_search_suggestion_format('item_id, name, pack_name, category'));
		$this->db->from('items');
		$this->db->where('deleted', $filters['is_deleted']);
		$this->db->where_in('item_type', $non_kit); // standard, exclude kit items since kits will be picked up later
		if($search){
		// advanced_search_filter: this was added to set the usage permission from the configuration modules and thus use the normal and fulltext search method  

	    $searchs=explode(' ', $search);		
          
		if($this->config->item('advanced_search_filter') == 1)
		{
			     $this->db->group_start();
		           foreach ($searchs as $key => $searchsf) {
		            $this->db->or_where('MATCH (name) AGAINST ("'.$this->db->escape_str($searchsf).'+" IN BOOLEAN MODE)', NULL, FALSE)> 0;
		            $this->db->like('name', $searchsf); // ospos search
		           } 
                  $this->db->group_end();
            ...
            $this->db->like('name', $search); // ospos search
        }  
		...
		{
			$suggestions[] = array('value' => $row->item_id, 'label' => $this->get_search_suggestion_label($row).' | '.$this->lang->line('items_category').': '.$row->category);
		}
		...
		foreach($this->db->get()->result() as $row)
		{
			$suggestions[] = array('value' => $row->item_id, 'label' => $this->get_search_suggestion_label($row).' | '.$this->lang->line('items_item_number').': '.$row->item_number);
		}

		if(!$input_search)  
		{
			//Search by category
		// 	$this->db->select('item_id, item_number, pack_name, name, category');
		// 	$this->db->from('items');
		// 	$this->db->where('deleted', $filters['is_deleted']);
		// 	$this->db->distinct();
		...
		// }
			...
			//Search by supplier
			$this->db->select('company_name, item_id, item_number, pack_name, name');
			$this->db->from('suppliers');
			$this->db->join('items AS items', 'suppliers.person_id = items.supplier_id');
			...
			foreach($this->db->get()->result() as $row)
			{
				...
				$suggestions[] = array('value' => $row->item_id, 'label' => $this->get_search_suggestion_label($row).' | '.$this->lang->line('items_supplier').': '.$row->company_name);
			}
